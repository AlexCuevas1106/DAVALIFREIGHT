...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/dashboard/:driverId", requireAuth, async (req, res) => {
    try {
      ...
      // Get current vehicle and trailer if assigned
      const currentVehicle = driver.currentVehicleId ? await storage.getVehicle(driver.currentVehicleId) : null;
      const currentTrailer = driver.currentTrailerId ? await storage.getTrailer(driver.currentTrailerId) : null;
      
      // Get hours of service
      const hos = await storage.getHoSByDriver(driverId);
      
      // Get pending inspections count
      const allInspections = await storage.getInspectionReports(driverId);
      const pendingInspections = allInspections.filter(i => i.status === 'pending').length;
      
      // Get documents count
      const documents = await storage.getDocumentFiles(driverId);
      const totalDocuments = documents.length;
      
      // Get recent activities
      const recentActivities = await storage.getActivityLogs(driverId, 5);
      
      // Get current shipment
      const shipments = await storage.getShipmentsByDriver(driverId);
      const currentShipment = shipments.find(s => s.status === 'in_transit') || null;
      ...
      const dashboardData = {
        driver: {
          ...
          dutyStartTime: driver.dutyStartTime?.toISOString() || null,
        ...
        currentShipment: currentShipment ? {
          ...
          shippingId: currentShipment.shippingId,
          destination: currentShipment.destination,
          actualDistance: currentShipment.actualDistance || 0
        } : null,
        hos: hos ? {
          remainingDriveTime: hos.remainingDriveTime,
          remainingDutyTime: hos.remainingDutyTime,
          drivingHours: hos.drivingHours,
          onDutyHours: hos.onDutyHours,
        ...
        metrics: {
          ...
          fuelEfficiency: 7.2,
          safetyScore: 98,
          hosCompliance: hos?.isCompliant ? 100 : 75
        }
      };
